...
public class RestaurantForm extends BaseForm {
    EncodedImage enc;
    Image imgs;
    ImageViewer imgv;
    ...
    public RestaurantForm(com.codename1.ui.util.Resources resourceObjectInstance) {
        initGuiBuilderComponents(resourceObjectInstance);
        restaurants = RestaurantService.getInstance().getRestaurants();

        getToolbar().setTitleComponent(
                FlowLayout.encloseCenterMiddle(
                        new Label("Restaurants", "Title"),
                        new Label(String.valueOf(restaurants.size()), "InboxNumber")
                )
        );
        ...
        Container destinationsList = new InfiniteContainer() {
            ...
            public Component[] fetchComponents(int index, int amount) {
                ...
                if(index + amount > restaurants.size()){
                    amount = restaurants.size() - index;
                ...
                for(int iter = 0; iter < amount; iter++){
                    ...
                    String url = Statics.BASE_URL+"/uploads/images/"+restaurants.get(offset).getImage();
                    ...
                    MultiButton mb = new MultiButton(restaurants.get(offset).getNomRestaurant());
                    mb.setTextLine2(restaurants.get(offset).getAdresseRestaurant());
                    mb.setTextLine3("Site: "+restaurants.get(offset).getSitewebRestaurant());
                             mb.setTextLine4("\n Specialité: \n" + restaurants.get(offset).getSpecialiteRestaurant());
                    ...
                    container.add(BorderLayout.NORTH, imgv);
                    container.add(BorderLayout.CENTER,mb);

                    more[iter] = container;
                    mb.addActionListener(e-> {
                        new UpdateRestaurantForm(restaurants.get(offset)).show();
                    });
